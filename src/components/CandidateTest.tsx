...
import showModalWith from './showModalWith';
...
const candidateTestData: { [key: string]: CandidateTestInfo } = {
  'organisation': {
    ...
    definition: (
      <>
        Where you see the word <strong>organisation</strong> used throughout this survey, it refers to <strong>[[ CLIENT ]]</strong>.
      </>
    ),
  },
  'exec': {
    ...
    definition: (
      <>
        <strong>Executive Management</strong> includes the most senior leaders in your organisation <br /> (e.g., CEO and the Leadership Team).
      </>
    ),
  },
  'seni': {
    ...
    definition: (
      <>
        <strong>Senior Management</strong> is defined as:
        <ul>
          <li>Chief Executive Officer - <em>Firstname Lastname</em></li>
          <li>Chief Operations Officer - <em>Firstname Lastname</em></li>
...
const CandidateTest: React.FC<Props> = ({
  children,
  targetParentDepth,
  targetSelector,
  replaceBy,
  ...
  }, [anchorId, byClick, popup, popupDiv]);
  ...
  }, [byClick, handleClick, handleTargetMouseOutOrUp, handleTargetMouseOverOrDown, targetParentDepth, targetSelector]);

  let CandidateTestStyle: CSSProperties = {};
  if (underline) {
  ...
  return (
    <span
      ref={CandidateTestRef}
      className='CandidateTest'
      style={CandidateTestStyle}
